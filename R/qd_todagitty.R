...
#'
#' @description Format an edgelist and send it to dagitty to identify variable
#'   adjustment sets.
#'
#' @param edgelist A vector of edge relationships. Must be strictly organized
#'   (see example for format).
#' @param diagram_type Character identifying the diagram type. Defaults to
#'   \code{"dag"}, but user can specify another graph type (see dagitty
#'   documentation).
#' @param showplot Logical indicating whether to produce a dagitty plot.
#'   Defaults to \code{FALSE}.
#' @param send_global Logical indicating whether to make the dagitty object
#'   available in the global environment. Defaults to \code{FALSE}.
#' @param dagitty_obj_name Character specifying the name of the dagitty object.
#'   Only used and required if \code{send_global = TRUE}.
...
#' @param ... Pass arguments to \code{adjustmentSets()}. See dagitty
#'   documentation for options.
#'
#' @note
#' The \code{exposure} and \code{outcome} options map to dagitty functions of
#'   the same name.
#'
#' @examples
#' edges <- c("A -> { B C D }",
